...
generate_deepstream_message_minimal (NvDsMsg2pCtx *ctx, NvDsEvent *events, guint size)
{
  ...
  for (i = 0; i < size; i++) {
    ...
    if (meta->otherAttrs){
        ss << meta->trackingId << "|" << object_enum_to_str (meta->objType, meta->objectId) << "|" << meta->otherAttrs << "|" << meta->bbox.left << "|" << meta->bbox.top
            << "|" << meta->bbox.left + meta->bbox.width << "|" << meta->bbox.top + meta->bbox.height;
    }else{
        ss << meta->trackingId << "|" << object_enum_to_str (meta->objType, meta->objectId) << "|" << meta->bbox.left << "|" << meta->bbox.top
            << "|" << meta->bbox.left + meta->bbox.width << "|" << meta->bbox.top + meta->bbox.height;
    }
  ...
  json_object_set_int_member (jobject, "cameraId", events[0].metadata->placeId);
  json_object_set_string_member (jobject, "pathFullView", events[0].metadata->ts);
  json_object_set_string_member (jobject, "pathPlate", events[0].metadata->videoPath);
